...
function isOneOfSizes(unit: Unit, sizes: VehicleSize[]) {
  return sizes.includes(unit.vehicleClass.size);
}

function isFlyer(unit: Unit) {
  return unit.special.find((s) => s.includes("Flyer")) !== undefined;
}

function isNotFlyer(unit: Unit) {
  return unit.special.find((s) => s.includes("Flyer")) === undefined;
}

function isNotFastMover(unit: Unit) {
  return unit.special.find((s) => s.includes("Fast Mover")) === undefined;
}

function isWalker(unit: Unit) {
  return unit.special.find((s) => s.includes("Walker")) !== undefined;
}

function isNotWalker(unit: Unit) {
  return unit.special.find((s) => s.includes("Flyer")) === undefined;
}

function hasMount(unit: Unit, mount: MountLocation) {
  return unit.mounts.find((m) => mount === m.type.mountType) !== undefined;
}

function hasAtLeastOneOfMounts(unit: Unit, mounts: MountLocation[]) {
  return (
